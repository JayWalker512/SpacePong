...
void GamePhysics(void)
{
	...
	for(i=0;i<2;i++)
	{
		if( (Ball.y + Ball.size / 2) > Paddle[i].y && (Ball.y + Ball.size / 2) < (Paddle[i].y + Paddle[i].height) )
		{
			if( (Ball.x + Ball.size / 2) > Paddle[i].x && (Ball.x + Ball.size / 2) < (Paddle[i].x + Paddle[i].width) )
			{
				...
				playchannel = SoundPlaySample(Samples.paddlehit); //play paddle hit sound
				//calc panning
				//FIXME: PANNING MATH IS NOT RIGHT
				float panning;
				panning = (Ball.x / GameState.resx);
				panning *= 2.0;
				panning -= 1.0;
	...
	if((Ball.y + Ball.size / 2) <= 0 || (Ball.y + Ball.size / 2) >= GameState.resy)
	{
		...
		playchannel = SoundPlaySample(Samples.wallhit); //wall hit sound
		//calculate panning
		//FIXME: PANNING MATH IS NOT RIGHT
		float panning;
		panning = (Ball.x / GameState.resx);
